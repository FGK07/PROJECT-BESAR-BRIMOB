...
// === Ambil data user ===
$stmt = $koneksi->prepare("SELECT * FROM users WHERE id = ? AND role = 'admin'");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['foto']['name'])) {
        ...
        $fileName = "admin_" . $admin_id . "_" . time() . ".jpg";
    ...
    // === Update data admin ===
    $stmt = $koneksi->prepare("UPDATE users SET nama = ?, email = ?, nomor_telepon = ?, alamat = ?, foto = ? WHERE id = ? AND role = 'admin'");
    $stmt->bind_param("sssssi", $nama, $email, $nomor_telepon, $alamat, $fotoBaru, $admin_id);
    if ($stmt->execute()) {
        // === Sinkronkan data session ===
        $_SESSION['admin']['nama'] = $nama;
        $_SESSION['admin']['email'] = $email;
        $_SESSION['admin']['nomor_telepon'] = $nomor_telepon;
        $_SESSION['admin']['alamat'] = $alamat;
        $_SESSION['admin']['foto'] = $fotoBaru;

        $_SESSION['flash'] = "Profil admin berhasil diperbarui!";
    }
...
<head>
    ...
    <title>Edit Profil Admin <?= htmlspecialchars($data['nama']) ?></title>
